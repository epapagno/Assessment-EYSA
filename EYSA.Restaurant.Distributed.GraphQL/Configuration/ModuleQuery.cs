...
namespace EYSA.Restaurant.Distributed.GraphQL.Configuration
{
    public class ModuleQuery : ObjectGraphType
    {
        ...
        private void AddFieldDish()
        {
            ...
            {
                FindDishSpecification filter = new FindDishSpecification(context.GetArgument<int>("id"));
        ...
        private void AddFieldIngredient()
        {
            ...
            {
                FindIngredientSpecification filter = new FindIngredientSpecification( context.GetArgument<int>("id"));
                using (var uow = IoCFactory.Instance.Container.Resolve<IModuleUoW>())
                {
        ...
        private void AddFieldAllergen()
        {
            ...
            {
                FindAllergenSpecification filter = new FindAllergenSpecification(context.GetArgument<int>("id"));
                using (var uow = IoCFactory.Instance.Container.Resolve<IModuleUoW>())
                {
